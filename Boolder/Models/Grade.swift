...
struct Grade: Comparable, Hashable, CustomStringConvertible, Strideable {
    ...
    static let min = Grade(Grade.grades.first!)
    static let max = Grade(Grade.grades.last!)
...
//    static let visibleGrades = ["1a", "1b", "1c", "2a", "2b", "2c", "3a", "3b", "3c", "4a", "4b", "4c", "5a", "5b", "5c", "6a", "6b", "6c", "7a", "7b", "7c", "8a", "8b", "8c", "9a"]
    ...
    init(index: Int) {
        string = Grade.grades[index % Grade.grades.count]
    }
    ...
    public func distance(to other: Grade) -> Grade.Stride {
        return Stride(other.index()) - Stride(self.index())
    }

    public func advanced(by n: Grade.Stride) -> Grade {
        let newIndex = Stride(self.index() + n)
